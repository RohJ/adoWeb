...
@Component({
  selector: 'app-gpcregister',
  templateUrl: './gpcregister.component.html',
  styleUrls: ['./gpcregister.component.scss']
})
export class GpcregisterComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  hashed='';
    ...
    registration: Registration = {
        ...
        type: '',
        amount: '0',
        txnid: 'GPC' + Math.floor(10000000000 + Math.random() * 90000000000),
        paym: '',
        status: 'Not Paid',
        agree: false,
    };
    ...
    ngOnInit() {
      ...
      this.titleService.setTitle("Global Pastor's Conference");
      this.metaTagService.updateTag({name: 'description', content: "Global Pastor's Conference Registration Form"});
      this.gpcForm = this.formBuilder.group (
        {
    ...
    public saveRegistration(): void {
      ...
      // console.log(this.gpcForm.value);
      ...
      this.registration = {
        ...
        arrivals: this.gpcForm.controls['arrival'].value,
        productinfo: this.gpcForm.controls['productinfo'].value,
        residential: this.gpcForm.controls['residential'].value,
        type: this.gpcForm.controls['type'].value,
        amount: this.registration.amount,
        txnid: this.registration.txnid,
        paym: this.registration.paym,
        status: this.registration.status,
        agree: this.gpcForm.controls['agree'].value,
      };
    ...
    } else if (this.gpcForm.controls['productinfo'].value == 'gpcsepbangalore2025' && this.gpcForm.controls['residential'].value == 'Non Residential') {
  ...
  } else if (this.gpcForm.controls['paym'].value == 'Part Payment'){
    if (age > 5) {
      amt = amt + 1000;
    } else {
      amt = amt + 0;
    ...
    private payuForm() :void {
        ...
        this.hashed = ('Z9XQW5'+'|'+this.registration.txnid+'|'+this.amnt+'|'+this.gpcForm.controls['productinfo'].value+'|'+this.registrationForm.at(0).get('firstname').value+'|'+this.registrationForm.at(0).get('email').value+'|'+udf1+'|'+udf2+'|'+udf3+'|'+udf4+'|'+udf5+'|'+'|'+'|'+'|'+'|'+'|'+this.salt);
      //  console.log(this.hashed);
        ...
        let productinfo = document.createElement('input');
          ...
          productinfo.value = this.gpcForm.controls['productinfo'].value;
  ...
  /*  register() {
    ...
    get registrationForm():FormArray{
      return <FormArray> this.gpcForm.get('registrationForm');
    }

    // convenience getter for easy access to form fields
    get f() { return this.gpcForm.controls; }
    ...
    public disableAdd() {
      if(this.gpcForm.controls['type'].value == 'Individual') {
        ...
        this.isDisabled = true;
      } else if(this.gpcForm.controls['type'].value == 'Married Couple') {
        if (this.count > 0) {
          for ( let j = 0; j < this.count;) {
          this.removePerson(this.count);
          }
        ...
        this.addPerson();
        this.isDisabled = true;
      } else if(this.gpcForm.controls['type'].value == 'Family' || this.gpcForm.controls['type'].value == 'Group') {
        if (this.count > 0) {
          for ( let j = 0; j < this.count;) {
          this.removePerson(this.count);
